...
def init_caribu(path_param='inputs', file='param_caribu.csv', meteo=None, nb_plantes=None, scenario=None):
    df = pd.read_csv(os.path.join(path_param, file),sep=';',header=0)
    param_init = dict(zip(df, df.iloc[0,:]))
    param_init.update({'radiation_interception':pd.DataFrame(), 'meteo':meteo, 'Ray':[0.]*nb_plantes, 'option_tiller_regression':scenario['option_tiller_regression'], 'option_mophogenetic_regulation_by_carbone':scenario["option_mophogenetic_regulation_by_carbone"]})
    return param_init
...
def apply_caribu_lgrass(lstring, lscene, TPS, current_day, tiller_appearance, nb_plantes, dico_caribu, day):
    if current_day > day:
        ...
        for id, v in res['Ei'].items():
            dico_caribu['Ray'][lstring[id][0].id_plante] += (
                    v * (res['area'][id] * 1E-6))  # v: MJ m-2, area: mm2, Ray: MJ PAR
